...
class Absorption():
    ...
    def excited_get(self,f_list,num):
        if 'Gaussian System' in f_list[0]:
    ...
    def spectrum(self,hami,dipole,dimention):
        ...
        delt = 0.0110*2 #the number of gaussian depression
        #delt = 0.0070*2 #the number of gaussian depression
        step = float((D[dimention-1]-D[0])/nstep)
        k = 0
        strengths = [] # y
        energy = [] # x
        lamda = [] # x 
        start = float(D[0]-200*step)
        end = float(D[dimention-1]+200*step)
        ...
        for i in np.arange(start, end , step):
            ...
            for j in range(dimention):
                ...
                print ("Dj",D[j],i,osc)
                flag = osc*(math.exp(-(D[j]-i)**2/(2*(delt**2))))
                num_strength = num_strength + flag
            #print (i/(2*math.sqrt(3.14)*delt))
            num_strength = num_strength * (i/(2*math.sqrt(3.14)*delt))
    ...
    def write2(self,data1,data2,filename):
        ...
        spe_file.close()
    #data writing --- energy+dipole
    def write3(self,data1,data2,data3,filename):
        aa = list(data1)
        bb = list(data2)
        cc = list(data3)
        spe_file = open(sys.argv[1].split('/')[0]+'_out'+'/'+str(filename),'w')
        for i in range(len(aa)):
            spe_file.write(str(aa[i])+'  '+'{:<8}'.format(str(bb[i]))+'{:<}'.format(str(cc[i]))+'\n')
        spe_file.close()
...
if __name__ == '__main__':
    ...
    MG_coord_list1 = []
    file_list = []
    b800_list = ['283','288','292','577','582','586','871','876','880']
    print ("\nlogfile list:")
        ...
        if file.endswith('.log'):
            ...
            for i in b800_list:
                if i in file: 
                    ...
                    xixi+=0 
            if xixi == 0:
                excited_list1.append(test.excited_get(ff_file,0.00))
            else:
                excited_list1.append(test.excited_get(ff_file,0.11))
                
            #print ('MG',MG_coord_list1)
            #print ('dipole',dipole_list1)
            #print ('exci',excited_list1)
    #print (excited_list1)
    #excited_list1 = [1.6555,1.6342,1.6591,1.6423,1.6544,1.6338,1.6511,1.6574,1.6386,1.6346,1.657,1.6213,1.6546,1.6446,1.6633,1.6432,1.6492,1.6467,1.6477,1.5714,1.5483,1.5772,1.565,1.564,1.5612,1.5707,1.5312]
    #excited_list1 = [1.7055,1.6342,1.6891,1.6123,1.6444,1.6338,1.5611,1.5874,1.6386,1.6346,1.657,1.6213,1.6546,1.6446,1.6633,1.6432,1.6492,1.6467,1.6477,1.5714,1.5483,1.5772,1.565,1.564,1.5612,1.5707,1.5312]
    #excited_list1 = [1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6,1.6]
    excited_list2 = [i * 8065.541154 for i in excited_list1]
    ...
    hami_mat_allcm = [] #Both the coupling values and the excited values are in cm-1
    for i in range(dimention1):
        ...
        for j in range(dimention1):
            ...
            if i == j:
                ...
                hami_mat_allcm.append(excited_list2[i])
            else:
    ...
    test.write(hami_mat_allcm2,'hamitonian.dat')    
    ...
    test.write3(file_list,excited_list1,dipole_list1,'excited+adjust_dipole.dat')

...
class Com_resource():
    ...
    def proc_thread_user(self,nodelist=[]):
        ...
        for n in nodelist:
            os.environ['n']=str(n)
            file = os.popen("timeout 15 ssh n$n 'top -d 0 -n 2 -b | grep -i 'day' -A 100 | grep -v -e '--' | tail -n 101;cat /proc/cpuinfo | grep 'processor' | wc -l'").readlines()
            if file:
                num_proc = int(file[-1])
                datalist = [] #toplist
                for f in file:
    ...
    def proc(self,nodelist=[]):
        ...
        for n in nodelist:
            os.environ['n']=str(n)
            file = os.popen("timeout 15 ssh n$n top -d 0 -n 2 -b | grep -i 'day' -A 100 | grep -v -e '--' | tail -n 101").readlines()
            if file:
                datalist = [] #toplist
                for f in file:
                    datalist.append(f.strip().split())
